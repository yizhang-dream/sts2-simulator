...
def create_fabricator(rng: Rng, ascension_level: int = 0) -> tuple[Creature, MonsterAI]:
    ...
    def _spawn_bot(combat: CombatState, creators) -> None:
        ...
        # F3 点4: C# Fabricator.cs:105 出怪选择 = base.RunRng.MonsterAi.NextItem(items)
        # （RunRng 整组引用进战斗, CombatState.cs:127）⇒ 抽 run 的 monster_ai 流;
        # combat.monster_ai_rng 无 run 上下文时按既有约定回退战斗内 rng。
        creator = combat.monster_ai_rng.choice(options)
