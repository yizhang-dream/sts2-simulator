...
from sts2_env.core.rng import Rng, UINT_MASK
...
class CombatState:
    ...
    def __init__(
        ...
        # C# CombatState 没有战斗级私有流——战斗内随机由两条途径承担:
        #   1) monster.RunRng = RunState.Rng（整组 run 流引用, CombatState.cs:127）；
        #   2) per-creature rng = 公式派生（CombatState.cs:136，不抽流）。
        # Python CombatState.rng 是 per-creature 结构缺位下的战斗级对齐位:
        # 有 run 上下文时种子按 C# 公式派生（combat_id 占位 0，坐标存在时公式不消费
        # combat_id）；无 run 上下文（独立对局/测试直构）回退传入的 rng_seed。
        derived_seed = self._derive_combat_rng_seed_from_run(
            getattr(player_state, "run_state", None), combat_id=0
        )
        self.rng = Rng(rng_seed if derived_seed is None else derived_seed)
        self.room = room
    ...
    @staticmethod
    def _derive_combat_rng_seed_from_run(run_state: object | None, combat_id: int) -> int | None:
        """C# CombatState.cs:136 per-creature rng 种子公式:

            monster.Rng = new Rng((uint)((RunState.Rng.Seed + CurrentMapCoord?.col)
                                         ?? ((long?)CurrentMapCoord?.row)
                                         ?? (CurrentActIndex + CombatId.Value)))

        逐项类型/空值语义（C# 原文）:
        - ``Rng.Seed`` 为 uint（Rng.cs:16）；``CurrentMapCoord`` 为 ``MapCoord?``
          （IRunState.cs:29），``?.col`` 仅在 coord 为 null 时为 null；
        - ``uint + int?`` 提升为 ``long?``（coord 为 null ⇒ 整体 null）；
        - ``(long?)CurrentMapCoord?.row`` 同受 coord 空值控制 ⇒ row 分支在 C# 中不可达；
        - 回退分支 ``int + uint`` 提升为 long；最外层 ``(uint)`` 取低 32 位。
        返回 None 表示无 run 上下文（rng set 缺失，公式无从套用）。
        """
        ...
        if rng_set is None:
            return None
        coords = getattr(run_state, "visited_map_coords", None) or []
        if coords:
            # C# RunState.cs:64-74: CurrentMapCoord = 最后访问的坐标，未访问为 null
            value = (int(getattr(rng_set, "seed")) & UINT_MASK) + int(coords[-1].col)
        else:
            value = int(getattr(run_state, "current_act_index", 0) or 0) + int(combat_id)
        return value & UINT_MASK

    def derive_creature_rng_seed(self, combat_id: int) -> int | None:
        """按 C# 公式派生 combat_id 对应生物的 rng 种子；无 run 上下文返回 None。

        注意: C# AttachCreature 的 CombatId 是含玩家的全局递增计数，Python
        add_enemy 赋 combat_id=len(self.enemies)——差异仅影响坐标为空的回退
        分支；真实对局必有坐标（走 Seed+col 分支，公式不消费 combat_id）。
        """
        state = getattr(self, "_primary_player_state", None)
        player_state = getattr(state, "player_state", None)
        run_state = getattr(player_state, "run_state", None)
        return self._derive_combat_rng_seed_from_run(run_state, combat_id=combat_id)

    def _spawned_enemy_hp_rng(self) -> Rng:
        """战斗中出怪的 HP 变体流引用。

        C# 战斗中出怪一律走 CombatState.CreateCreature（SurprisePower.cs:22-23 /
        InfestedPower.cs:30-32 经 CreatureCmd.Add → CreateCreature），敌方生物的
        HP 变体由 SetUniqueMonsterHpValue(creaturesOnSide, RunState.Rng.Niche)
        消费 run 的 Niche 流（CombatState.cs:132；每只生物恰好一抽）。Python
        create_* 工厂把 HP 掷进传入的 rng 且无 unique HP 变体逻辑 ⇒ 以 run 的
        niche 流引用对齐（同流同序；缺"同侧已有 maxHp 去重"一步，记录为内容
        缺口）。这两条路径为固定类型出怪，无 MonsterAi 选择。无 run 上下文时
        _run_rng 按既有约定回退战斗内 rng（独立对局确定性不变）。
        """
    ...
    def combat_player_state_for(self, creature: Creature) -> CombatPlayerState | None:
        # 战斗奖励拾取遗物路径会把 PlayerState(持久态)传进来:
        # run_state.obtain_relic_with_setup → after_obtained(owner=PlayerState),
        # 若 PlayerState 仍挂着刚结束战斗的 combat_state 引用,钩子会走战斗内
        # 分支以 PlayerState 调用本方法(dict 键是 Creature → unhashable 崩)。
        # 单玩家语义下映射回 root player。
        if getattr(creature, "combat_state", None) is self and not isinstance(
                creature, Creature):
            creature = self._root_player
    ...
    def _continue_player_turn_setup(self, player_index: int = 0, stage: str = "block") -> None:
        ...
        for state in self.combat_player_states:
            if state.orb_queue is not None:
                ...
                # 此处原有一次 _check_combat_end。C# 在玩家侧 StartTurn
                # 的 hook 派发中途不判终局,唯一检查点在全部启动 hook 之后
                # (CombatManager.cs:830);中途的即时终局改由动作层
                # deal_damage/kill_creature 内的检查覆盖(对应 C#
                # ActionExecutor.cs:161-171 的逐动作检查)。
                if self.pending_choice is not None or self.is_over:
                    return
        for state in self.combat_player_states:
    ...
    def _prepare_opening_draw_for_owner(self, owner: Creature, draw_count: int) -> int:
        """Round-1 draw-pile setup from C# `SetupPlayerTurn()`."""
        state = self.combat_player_state_for(owner)
        if state is None:
        ...
        # 对齐 CombatManager.cs:908-923:先 ShouldStartAtBottomOfDrawPile
        # 卡逐张 MoveToBottomInternal(保序落到堆底),再对 Innate 卡
        # (Except 置底卡)MoveToTopInternal,handDraw=max(handDraw, innate数)
        # 且 clamp ≤ MaxCardsInHand。
        bottom_cards = [
            card for card in state.draw
            if should_start_at_bottom_of_draw_pile(card)
        ]
        bottom_ids = {id(card) for card in bottom_cards}
        remaining = [card for card in state.draw if id(card) not in bottom_ids]

        innate_cards = [card for card in remaining if card.is_innate]
        non_innate_cards = [card for card in remaining if not card.is_innate]
        state.draw[:] = innate_cards + non_innate_cards + bottom_cards
    ...
    def end_player_turn(self) -> None:
        ...
        for state in self.combat_player_states:
            if state.orb_queue is not None:
                ...
                # C# DoTurnEnd(CombatManager.cs:1602-1628)在 orb
                # BeforeTurnEnd 之后无 CheckWinCondition,只在 DoTurnEnd 整体
                # 结束后(CombatManager.cs:1579)检查;Python 的对应检查在下方
                # _resolve_end_of_turn_hand 之后。
                if self.is_over:
                    return
        ...
        # C# 玩家回合 PhaseTwo(AfterSideTurnEnd 之后)无
        # CheckWinCondition(CombatManager.cs:1741-1778);终局改由敌方回合
        # 开始节点(下方 _execute_enemy_turn 的检查,对应 CombatManager.cs:847)
        # 或额外回合的 StartTurn 检查(CombatManager.cs:830)捕获。
        if self.is_over:
            return
    ...
    def _finish_enemy_turn(self, *, resume_player_turn: bool = False) -> None:
        ...
        # C# EndEnemyTurnInternal(BeforeSideTurnEnd→cleanup→
        # AfterSideTurnEnd, CombatManager.cs:1691-1700)中途无检查,唯一检查在
        # EndEnemyTurn 整体之后(CombatManager.cs:1089);Python 对应检查在本
        # 函数尾部 after_turn_end 派发之后。
        if self.is_over:
            return
    ...
    def spawn_surprise_replacements(self, owner: Creature) -> list[Creature]:
        ...
        sneaky, sneaky_ai = create_sneaky_gremlin(self._spawned_enemy_hp_rng())
        self.add_enemy(sneaky, sneaky_ai)
        spawned.append(sneaky)

        fat, fat_ai = create_fat_gremlin(self._spawned_enemy_hp_rng())
    ...
    def _check_combat_end(self) -> None:
        ...
        # C# CheckWinCondition 语义(CombatManager.cs:648-661):pendingLoss
        # (玩家死)先于"敌全灭"判胜 → 同归局(玩家+全敌同帧死)判负。
        # 复活(FairyInABottle/should_die_late)在 kill_creature 内先于本检查
        # 结算,不受此顺序影响。
        if self.primary_player.is_dead:
            self._end_combat(player_won=False)
        elif not self.alive_enemies and not blocking_dead_enemies:
    ...
    def _end_combat(self, player_won: bool) -> None:
        ...
        # 对齐 C# 触发时机:Hook.AfterCombatEnd 只在 EndCombatInternal
        # (CombatManager.cs:1318)触发,而它只在胜利路径
        # (CheckWinCondition→IsCombatEnding, CombatManager.cs:1390-1403)可达;
        # 败场 ProcessPendingLoss(CombatManager.cs:1277-1286)只置
        # IsInProgress=False 并发 CombatEnded 事件,不走 AfterCombatEnd。
        if player_won:
