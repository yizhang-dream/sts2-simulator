...
# C# EnchantmentModel.ShouldStartAtBottomOfDrawPile(EnchantmentModel.cs:125)
# 基类默认 False;反编译树唯一覆盖者 = Imbued(Imbued.cs:9)。
# CombatManager.cs:908-923:玩家首回合(TurnNumber==1)先把带该标志的卡移到
# 抽牌堆底,再把 Innate(排除置底卡)移顶,handDraw=max(handDraw, innate数)
# 且 clamp ≤ MaxCardsInHand(10)。
SHOULD_START_AT_BOTTOM_OF_DRAW_PILE = frozenset({"Imbued"})
...
def should_start_at_bottom_of_draw_pile(card: CardInstance) -> bool:
    """Card carries an enchantment that sorts to the draw-pile bottom on round 1."""
    return any(
        name in SHOULD_START_AT_BOTTOM_OF_DRAW_PILE
        for name in card.enchantments
    )
