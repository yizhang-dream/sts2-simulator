...
def deal_damage_to_random_enemy(
    ...
    ``allow_duplicates=False`` mirrors the C# AttackCommand builder flag
    ``TargetingRandomOpponents(state, allowDuplicates)``(AttackCommand.cs:322,
    默认 True):每跳先把本攻击已命中者从候选池剔除,剔空则抛错
    (AttackCommand.cs:609-616 的 InvalidOperationException 同义);候选池本就
    为空(无存活目标)仍维持 break 回退(AttackCommand.cs:557-563)。
    当前反编译树无任何调用方传 False,全部卡池走默认允许重复路径。
    """
    results = []
    already_hit: list[Creature] = []
    with combat.attack_context(dealer, None, props):
        for _ in range(hits):
            if dealer.is_dead:
                break
            ...
            if not allow_duplicates:
                hittable = [enemy for enemy in hittable if enemy not in already_hit]
                if not hittable:
                    raise ValueError(
                        "No valid targets for random attack with duplicates "
                        "disallowed (C# AttackCommand throws here)."
                    )
