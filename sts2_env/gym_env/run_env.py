...
# 引擎步预算熔断异常(engine_guard 模块)必须穿透下方 step() 的
# ``except Exception`` 兜底——否则熔断局被吞成整局失败(reward=-1 死亡标签)。
# 守卫缺失时为 None,兜底语义不变。
try:
    from sts2_env.engine_guard import \
        EngineStepBudgetExceeded as _EngineStepBudgetExceeded
except Exception:  # pragma: no cover - engine_guard 不可用时
    _EngineStepBudgetExceeded = None
...
class STS2RunEnv(gymnasium.Env):
    ...
    def step(
        ...
        try:
            ...
            # 引擎步熔断异常穿透本兜底(先于兜底判定),由护栏统一记账、
            # 分支判 failed;不得吞成整局失败、污染终局判定。
            if (_EngineStepBudgetExceeded is not None
                    and isinstance(exc, _EngineStepBudgetExceeded)):
                raise
