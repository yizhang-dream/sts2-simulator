...
class TestFixedRotation:
    ...
    def test_fabricator_spawn_history_is_shared_across_bot_pools(self):
        ...
        combat.add_enemy(fabricator, fabricator_ai)
        # (2026-09-06): C# Fabricator.cs:105 出怪选择抽 run 的 MonsterAi 流;
        # 本测试无 run 上下文 ⇒ combat.monster_ai_rng 按约定回退战斗级 rng,
        # 选择序列随之漂移(预期行为): 第二次 fabricate 的两次选择均复抽
        # NOISEBOT/STABBOT(全局 last-spawn 排除只作用于"全局最后一个"=上一次
        # aggro 选择, defense 池不受限; 第二次 aggro 的 last 已被 defense 选择
        # 覆盖 ⇒ 池不收敛为单选项, 复抽合法)。
        expected_first_fabricate = [NOISEBOT_MONSTER_ID, STABBOT_MONSTER_ID]
        expected_second_fabricate = [NOISEBOT_MONSTER_ID, STABBOT_MONSTER_ID]
