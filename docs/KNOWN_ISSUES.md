...
Current known issues, bugs, and limitations of the STS2 simulator project.
...
### 7. Full-run policies need far more than the random baseline

**Severity:** High (policy challenge)

**Problem:** Completing a full run requires a competent policy. The
full-run environment's untrained random-policy baseline is very low: random
play typically progresses only a few floors into Act 1 and does not complete
runs.
...
**Mitigation:** Reward-shaping hooks exist in the environment but only
provide small floor-progression bonuses. Reliable full-run completion
requires a fundamentally stronger policy approach (e.g., hierarchical
policies or curriculum-style exploration).

### 8. Combat-layer parity is only validated for the Ironclad character so far

**Severity:** Medium

**Problem:** The combat-layer parity suites only exercise Ironclad starter decks. All combat-side parity testing uses the Ironclad character.

**Impact:** Coverage for the other characters is not yet validated. Silent, Defect, Necrobinder, and Regent differ in ways that are not covered by the current suites:
...
**Workaround:** The simulator supports all 5 characters (cards, powers, monsters are all implemented). Parity suites need to be extended to cover character selection.
...
**Workaround:** Start the game first, wait for the main menu to appear, then start the Python agent. The bridge client has reconnection retry logic (multiple attempts with delay).
